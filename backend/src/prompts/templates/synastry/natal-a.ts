/**
 * 合盘中 A 的本命分析 Prompt（中国本土化版本）
 *
 * 输出：从 A 的视角看 ta 在关系中是什么样的人
 */
...
import { resolveNameA, resolveRelationType } from './utils';
...
export const synastryNatalAPrompt: PromptTemplate = {
  meta: {
    id: 'synastry-natal-a',
    version: '5.0',
    module: 'synastry',
    priority: 'P1',
    description: 'A 的人设分析（支持多种关系类型）',
  ...
  user: (ctx: PromptContext) => {
    const name = resolveNameA(ctx);
    const chartData = ctx.chartA || ctx.chart_a || ctx.chart;
    const gender = ctx.genderA || ctx.gender_a || ctx.gender || '';
    const hasBirthTime = ctx.has_birth_time_a !== false && ctx.hasBirthTimeA !== false;
...
// 注册
registry.register(synastryNatalAPrompt);
