...
## 节气与节日融入
如果「时令参考」中包含节气信息，在 summary 中自然融入节气氛围。如："大寒时节，内心戏更重了，适合安静独处"、"立春前后，新计划的灵感已经在酝酿了"。「时令参考」中没有节气则忽略。
如果「时令参考」中包含节日信息，在 summary 和 advice 中融入节日氛围。如：春节前后可建议"给自己设个新年flag"，中秋可关注"家庭关系和归属感"。
❌ 如果「时令参考」中没有提到节日，则绝对不要提及任何节日名称或节日相关内容——即使日期看起来临近某个节日也不要提及，因为该节日可能已经过去。
