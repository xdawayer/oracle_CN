...
import { isDatabaseConfigured, query, getOne, upsert } from '../db/mysql.js';
...
async function saveOrderToDB(order: WxPayOrder): Promise<void> {
  ...
  try {
    ...
    }, ['user_id', 'order_type', 'plan', 'points_amount', 'total_fee', 'status', 'transaction_id', 'paid_at']);
  } catch (err) {
...
async function loadOrderFromDB(orderId: string): Promise<WxPayOrder | null> {
  ...
  try {
    const data = await getOne<Record<string, unknown>>(
      'SELECT * FROM wxpay_orders WHERE order_id = ?',
      [orderId]
    );
    if (!data) return null;
    return {
      ...
      orderType: data.order_type as 'subscription' | 'points',
      plan: (data.plan as string) || undefined,
      pointsAmount: (data.points_amount as number) || undefined,
      totalFee: data.total_fee as number,
      status: data.status as WxPayOrder['status'],
...
async function getUserOpenid(userId: string): Promise<string> {
  if (!isDatabaseConfigured()) return 'dev_openid';
  try {
    const data = await getOne<{ wechat_openid: string | null }>(
...
const wxpayService = {
    ...
    if (isDatabaseConfigured()) {
      try {
        const data = await query<Record<string, unknown>>(
          'SELECT * FROM wxpay_orders WHERE user_id = ? ORDER BY created_at DESC LIMIT 50',
          [userId]
        );
  ...
  async closeExpiredOrders(): Promise<number> {
    ...
    try {
      const pendingOrders = await query<{ order_id: string }>(
        'SELECT order_id FROM wxpay_orders WHERE status = ? AND created_at < ? LIMIT 50',
        ['pending', thirtyMinAgo]
      );
      ...
      for (const row of pendingOrders) {
        ...
        try {
          // 先查询微信确认订单确实未支付
          const queryResult = await this.queryWxPayOrder(currentOrderId);
          ...
          // 调用微信关闭订单 API
          if (isWxPayConfigured()) {
            const closeUrl = `/v3/pay/transactions/out-trade-no/${currentOrderId}/close`;
            const closeBody = JSON.stringify({ mchid: wxpayConfig.mchId });
            await wxpayRequest('POST', closeUrl, closeBody);
          }

          // 更新本地状态为 closed
          const existingOrder = orderCache.get(currentOrderId) || await loadOrderFromDB(currentOrderId);
          if (existingOrder && existingOrder.status === 'pending') {
            existingOrder.status = 'closed';
            orderCacheSet(currentOrderId, existingOrder);
            await saveOrderToDB(existingOrder);
            closedCount++;
          }
        } catch (error) {
          console.error(`[wxpay] 关闭超时订单失败: ${currentOrderId}`, error);
        }
      }
