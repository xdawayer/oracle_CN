...
export function generateMajorEvent(
  ...
  if (isJupiterReturn) {
    return {
      ...
      impact: 15,
      description:
        '每12年一次的木星回归是您的"幸运年"！木星带来扩张、乐观、机遇的能量。这一年适合开拓新领域、学习新技能、扩展人脉、远行、出版等。但要注意避免过度乐观导致的承诺过多。',
    };
  }
  ...
  if (isUranusOpposition) {
    return {
      ...
      impact: -10,
      description:
        '这就是常说的"中年危机"的占星根源，但更准确地说是"中年觉醒"。内心深处那个"真正的自己"开始敲门，要求被看见、被表达。您可能突然感到不满足，渴望自由与改变。不要压抑这种感觉，而是找到健康的方式去探索"我是谁"。',
    };
  }
...
export function generateAstroSummary(year: number, age: number): string {
  ...
  return `这一年，过境木星行经您的${houses[houseIdx]}（也就是掌管${houseThemes[houseIdx]}的领域），这意味着宇宙正在为你这个方向注入扩张和成长的能量。${houseAdvices[houseIdx]}`;
}

/**
 * 生成八字摘要
 */
export function generateBaziSummary(ganzhi: GanZhi): string {
  const summaries = [
    `${ganzhi.full}年，天干${ganzhi.stem}透出，地支${ganzhi.branch}值年。对于您的日主而言，流年带来的是调整与适应的能量。`,
    `从八字角度看，${ganzhi.full}年流年与您的命局形成一定的互动，需要在行动与守成之间把握平衡。`,
    `${ganzhi.full}年的能量特质与您的本命配置有所互补，适合在既有基础上稳步发展。`,
    `流年${ganzhi.full}与您的大运形成协同，整体运势呈现积极向上的态势。`,
  ];
