<!-- INPUT: 后端业务服务目录结构与输出索引（含 GM 积分消费、报告积分计价、AI 流式与 token 预算）。 -->
<!-- OUTPUT: services 架构摘要与文件清单（含 AI 流式调用、token 预算与地理搜索多语言过滤记录）。 -->
