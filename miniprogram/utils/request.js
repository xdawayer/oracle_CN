...
const wxRequest = (options) => new Promise((resolve, reject) => {
  const isPhone = getEnvVersion() !== 'develop';
  ...
  if (_useCloud()) {
    ...
    const fallback = (reason, isCloudBroken) => {
      ...
      settled = true;
      if (isCloudBroken) {
        // 手机上冷却时间缩短到 5s（wx.request 在手机上不可用，冷却太长会卡死所有请求）
        _cloudFailedUntil = Date.now() + (isPhone ? 5000 : CLOUD_COOLDOWN_MS);
      }
      if (isCloudBroken && !isPhone) {
        // 仅在开发者工具中降级到 wx.request（可勾选"不校验域名"）
        logger.warn('[request] callContainer ' + reason + ', fallback to wx.request (cooldown 30s)');
        _directRequest(options, resolve, reject);
      } else {
        // 手机上 or 超时：直接 reject 并保留原始错误信息，让上层 retry 仍走 callContainer
        logger.warn('[request] callContainer ' + reason + ', reject');
  ...
  // _useCloud() 返回 false：手机上不应走 wx.request（域名未白名单）
  if (isPhone) {
    logger.warn('[request] cloud unavailable on phone, reject');
    reject({ errMsg: 'Cloud hosting unavailable (cooldown or wx.cloud missing)' });
    return;
  }
  _directRequest(options, resolve, reject);
});
